# Нормализация таблицы «Успеваемость» до 3НФ

## Часть 1. Теоретический анализ

### 1. Определение первичного ключа

Исходная таблица содержит информацию о студенте, его группе, преподавателе, предмете, дате экзамена и оценке.
...
* `subject_id` не подходит, потому что один предмет сдают разные студенты;
* `teacher_id` не подходит, потому что один преподаватель может работать с несколькими студентами;
* `exam_date` не подходит, потому что в один день могут сдавать экзамен несколько студентов;
* `(student_id, subject_id)` теоретически не подходит, если студент может сдавать один и тот же предмет повторно в разные даты.

Поэтому для данной таблицы подходит составной первичный ключ:
...
В исходной таблице существуют следующие функциональные зависимости:
...
Также для каждой записи ключ определяет остальные данные:
...
При этом предполагается, что каждый предмет закреплён за одним преподавателем:
...
Именно из-за этих зависимостей исходную таблицу необходимо разделить на несколько таблиц.

---

### 4. Нормальная форма исходной таблицы

Исходная таблица находится в **первой нормальной форме (1НФ)**.

Причина:

* все значения являются атомарными;
...
При этом некоторые атрибуты зависят только от части составного ключа.
...
Это частичные функциональные зависимости.

Следовательно:

**Исходная таблица находится в 1НФ, но не находится во 2НФ и 3НФ.**
...
Groups
   ...
   └── 1 : N ── Students
                    │
                    └── 1 : N ── StudentGrades ── N : 1 ── Subjects
                                                        │
                                                        └── N : 1 ── Teachers
```
